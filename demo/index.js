/**
 * Created by beizhu on 2019/2/20.
 */
const path = require('path');
const fs = require('fs');
const babel = require('babel-core');
const jsxToSchemaPlugin = require('../src');
const stage0 = require('babel-preset-stage-0');

const result = babel.transform(getCode(), {
  presets: [stage0],
  plugins: [jsxToSchemaPlugin],
});

fs.writeFileSync(path.join('./demo/schema'), toJson(result.code));

console.log(toJson(result.code));

function toJson(code) {
  try {
    let input = '[' + code + ']';
    return JSON.stringify(eval(input)).slice(1, -1);
  } catch (e) {
    return code;
  }
}
